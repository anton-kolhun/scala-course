...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def descendingByRetweet: TweetList = {
    val minElement = getMostLeftElement(elem)
    new Cons(minElement, remove(minElement).descendingByRetweet)
  }

  def getMostLeftElement(currentMin: Tweet): Tweet = left match {
    case nonEmpty: NonEmpty => nonEmpty.getMostLeftElement(elem)
    case _: Empty => elem
  }
